...
Template comes with no endpoints configured. Add your API routes:

```bash
# Edit the script and add test_endpoint calls:
test_endpoint "Health Check" "/api/health"
test_endpoint "Users API" "/api/users"
test_endpoint "Status" "/api/status" 404  # Expected 404
```

**Customize for:**

- Your API endpoints (REQUIRED)
- Default base URL (match your server port)
- Expected status codes (not all are 200)
- POST/PUT endpoint tests with request bodies
